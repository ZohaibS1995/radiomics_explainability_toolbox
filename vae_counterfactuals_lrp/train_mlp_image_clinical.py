...
class ClassificationDataset_clinical(Dataset):
...
class MLP(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Linear(512 + 21, 256)
...
if __name__ == "__main__":
    ...
    early_stopping = EarlyStopping(patience=100, verbose=True,  path=os.path.join(config.model_save_dir, f"{fold_no}_mlp_checkpoint.pt"))

    training_dataset = ClassificationDataset_clinical(train_latent_space, train_labels)
    training_dataloader = DataLoader(training_dataset, batch_size=16, shuffle=True, num_workers=0)

    validation_dataset = ClassificationDataset_clinical(val_latent_space, val_labels)
